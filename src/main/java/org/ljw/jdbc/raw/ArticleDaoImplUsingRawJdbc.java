...
public class ArticleDaoImplUsingRawJdbc implements ArticleDao {
	...
	public ArticleDaoImplUsingRawJdbc() {
		ds = new MariaDbDataSource("jdbc:mariadb://localhost:3306/jwdb?user=jw&password=2515");
	}
	...
	public List<Article> listArticles() throws DaoException {
		try (Connection con = ds.getConnection(); PreparedStatement ps = con.prepareStatement(LIST_ARTICLES)) {
	...
	public Article getArticle(String articleId) throws DaoException {
		try (Connection con = ds.getConnection(); PreparedStatement ps = con.prepareStatement(GET_ARTICLE)) {
	...
	public void addArticle(Article article) throws DaoException {
		try (Connection con = ds.getConnection(); PreparedStatement ps = con.prepareStatement(ADD_ARTICLE)) {
	...
	public void updateArticle(Article article) throws DaoException {
		try (Connection con = ds.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_ARTICLE)) {
			ps.setString(1, article.getTitle());
			ps.setString(2, article.getContent());
	...
	public void deleteArticle(String articleId) throws DaoException {
		try (Connection con = ds.getConnection(); PreparedStatement ps = con.prepareStatement(DELETE_ARTICLE)) {
			ps.setString(1, articleId);
			ps.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
